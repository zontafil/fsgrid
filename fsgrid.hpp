...
template <typename T, int stencil> class FsGrid {
   ...
   FsGrid(std::array<int32_t,3> globalSize, MPI_Comm parent_comm, std::array<bool,3> isPeriodic, FsGridCouplingInformation& coupling)
      ...
      /*! Create the neccessary mesh datastructures inside a vlsv file to directly
       * write FsGrid data into it.
       *
       * \param vlsvWriter vlsv file object to write into
       * \param meshName identifying string of the resulting mesh structure
       */
      void createVlsvMesh(vlsv::Writer& vlsvWriter, const std::string& meshName) {
         ...
         for(int z=0; z<localSize[2]; z++) {
            for(int y=0; y<localSize[1]; y++) {
               for(int x=0; x<localSize[0]; x++) {
                  globalIds[i++] = (localStart[2]+z)*globalSize[0]*globalSize[1]+
                                   (localStart[1]+y)*globalSize[0] +
                                   localStart[0]+x;
               }
            }
         }
      ...
      void vlsvOutputVariable(vlsv::Writer& vlsvWriter, std::function<double(T&)>& func, const std::string& meshName, const std::string& variableName) {
         ...
         std::vector<double> varBuffer(storageSize);

         for(int z=0; z<localSize[2]; z++) {
            for(int y=0; y<localSize[1]; y++) {
               for(int x=0; x<localSize[0]; x++) {
                  varBuffer[z*localSize[1]*localSize[0]+y*localSize[0]+x]=func(*get(x,y,z));
               }
            }
         }
